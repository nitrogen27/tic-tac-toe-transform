...
@pytest.mark.asyncio
async def test_finish_game_auto_train_rejects_duplicate_variant(monkeypatch) -> None:
    ws = DummyWebSocket()
    monkeypatch.setattr(handler, "_is_training_active", lambda variant: True)
...
async def test_generate_engine_dataset_dispatch(monkeypatch) -> None:
    ...
    async def fake_generate_engine_dataset(variant: str, count: int, cb):
        return f"C:/tmp/{variant}_engine.json"
